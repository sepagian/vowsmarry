...
export const GET: RequestHandler = async ({ url, locals: { supabase } }) => {
	...
	if (error) {
		...
		if (error.message.includes('expired')) {
			throw redirect(
				303,
				'/error?type=expired_verification_link&message=Your email verification link has expired',
			);
		} else if (error.message.includes('invalid')) {
			throw redirect(
				303,
				'/error?type=invalid_verification_link&message=Invalid verification token',
			);
		} else {
	...
	if (type === 'email') {
		...
		redirectTo.searchParams.set(
			'message',
			'Email verified for password reset. You can now set your new password.',
		);
