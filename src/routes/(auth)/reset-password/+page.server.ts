...
export const load: PageServerLoad = async ({ locals: { user, supabase }, url }) => {
	...
	if (!token_hash) {
		...
		redirect(302, '/error?type=invalid_reset_link&message=Invalid password reset link. Please request a new one.');
	}

	if (type && type !== 'recovery') {
		console.error('Invalid type parameter in reset password URL:', type);
		redirect(302, '/error?type=invalid_reset_link&message=Invalid password reset link type. Please request a new one.');
	}
	...
	default: async ({ request, locals: { supabase }, url }) => {
		...
		try {
			...
			if (!session && token_hash) {
				...
				if (verifyError) {
					...
					if (verifyError.message.includes('expired') || verifyError.message.includes('Token has expired')) {
						redirect(302, '/error?type=expired_reset_link&message=Your password reset link has expired. Please request a new one.');
					} else if (verifyError.message.includes('invalid') || verifyError.message.includes('Token is invalid')) {
						redirect(302, '/error?type=invalid_reset_link&message=Invalid reset token. Please request a new password reset.');
					} else {
						redirect(302, '/error?type=invalid_reset_link&message=Unable to verify reset token. Please try again.');
					}
				}
			...
			if (!sessionEstablished) {
				...
				redirect(302, '/error?type=invalid_reset_link&message=Invalid reset session. Please request a new password reset.');
			}
			...
			if (updateError) {
				...
				if (updateError.message.includes('Too many requests') || updateError.message.includes('rate limit')) {
					return fail(429, {
						form,
					...
					redirect(302, '/error?type=invalid_reset_link&message=Your reset session has expired. Please request a new password reset.');
				} else {
					return fail(400, {
						form,
