<script lang="ts">
	...
	const style = categoryStyles[error.category as keyof typeof categoryStyles] || categoryStyles.generic;
...
<div class="flex flex-col gap-8 max-w-md mx-auto">
	...
	<div class="flex flex-col items-center gap-4 text-center">
		<div class="relative">
			<div class="mx-auto w-16 h-16 {style.bgColor} rounded-full flex items-center justify-center mb-2 shadow-sm">
				<div class="{error.icon} w-8 h-8 {style.iconColor}"></div>
			</div>
			<!-- Category Badge -->
			<div class="absolute -bottom-1 -right-1">
				<Badge variant={style.badgeVariant} class="text-xs px-2 py-1">
					{categoryLabel}
				</Badge>
			</div>
		</div>
