...
export const load: PageServerLoad = async ({
	...
	const sortBy = url.searchParams.get("sortBy") || "documentDate";
	const sortOrder = url.searchParams.get("sortOrder") || "asc";

	const documentForm = await superValidate(valibot(documentSchema));

	const [documentList, documentCount] = await Promise.all([
		plannerDb
		...
		async ({ organizationId, plannerDb }, { request }) => {
			try {
				...
				try {
					...
					return {
						form: { ...form, data: formDataWithoutFile },
						success: true,
