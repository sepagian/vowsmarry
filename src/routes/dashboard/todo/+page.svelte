						...
						<div class="flex-1 min-w-0">
							...
							<div class="flex items-center gap-4 text-sm text-muted-foreground">
								<div class="flex items-center gap-1">
									...
									<span class="{isOverdue(todo.dueDate?.toISOString()?.split('T')[0] || null, todo.status) ? 'text-red-600 font-medium' : ''}">
										Due: {formatDate(todo.dueDate?.toISOString() || null)}
										{#if isOverdue(todo.dueDate?.toISOString()?.split('T')[0] || null, todo.status)}
											(Overdue)
										{/if}
									</span>
								</div>
