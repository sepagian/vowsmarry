<script lang="ts">
  ...
  import { useWorkspace } from "$lib/query/workspace";
  ...
  const weddingDate = $derived(
    workspaceQuery.data?.weddingDate
      ? new Date(workspaceQuery.data.weddingDate)
      : null,
  );
  ...
  <div class="flex justify-between gap-2 px-4 align-center">
    <div class="flex flex-col gap-2">
      {#if workspaceQuery.data}
        {@const groomName = workspaceQuery.data.groomName}
        {@const brideName = workspaceQuery.data.brideName}
