...
export class RateLimiter {
	...
	cleanupExpired(): void {
		...
		// If cache is too large, clean up more aggressively
		const shouldCleanAll = this.cache.size > MAX_CACHE_SIZE;

		for (const [key, entry] of this.cache.entries()) {
			if (shouldCleanAll || entry.resetTime <= now) {
				this.cache.delete(key);
				cleaned++;
				// Stop early if we've cleaned enough
				if (!shouldCleanAll && cleaned >= 1000) break;
			}
		}

		if (cleaned > 0) {
			console.log(
				`[RateLimit] Cleaned up ${cleaned} expired entries (cache size: ${this.cache.size})`
			);
		}
	}
