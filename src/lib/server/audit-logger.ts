...
export class AuditLogger {
	...
	async log(entry: AuditLogEntry): Promise<void> {
		try {
			// Limit changes object size to prevent excessive JSON.stringify operations
			let changesJson: string | null = null;
			if (entry.changes) {
				// Limit to 100 key-value pairs and truncate large values
				const limitedChanges: Record<string, any> = {};
				...
				changesJson = JSON.stringify(limitedChanges);
			}
