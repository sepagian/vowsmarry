...
export type ErrorCategory = 'password_reset' | 'email_verification' | 'authentication' | 'network' | 'server' | 'generic';
...
export const ERROR_CONFIGS: Record<string, ErrorConfig> = {
	...
	invalid_reset_link: {
		...
		description: 'This password reset link is invalid or has already been used. Reset links can only be used once for security.',
	...
	expired_reset_link: {
		...
		description: 'This password reset link has expired. For security reasons, reset links are only valid for 24 hours.',
	...
	reset_link_error: {
		...
		description: 'There was an issue processing your password reset link. This may be due to the link being malformed or corrupted.',
		icon: 'i-lucide:link-off',
		category: 'password_reset',
		actions: [
			{ label: 'Request New Reset Link', href: '/forgot-password', variant: 'default' },
	...
	invalid_verification_link: {
		...
		description: 'This email verification link is invalid or has already been used. If you already verified your email, you can log in normally.',
	...
	expired_verification_link: {
		...
		description: 'This email verification link has expired. Verification links are valid for 24 hours after registration.',
		icon: 'i-lucide:clock-x',
		category: 'email_verification',
		actions: [
			{ label: 'Back to Login', href: '/login', variant: 'default' },
	...
	verification_link_error: {
		...
		description: 'There was an issue processing your email verification link. The link may be malformed or corrupted.',
		icon: 'i-lucide:mail-x',
		category: 'email_verification',
		actions: [
			{ label: 'Back to Login', href: '/login', variant: 'default' },
	...
	auth_error: {
		...
		description: 'There was a problem with your authentication. This could be due to invalid credentials or account issues.',
	...
	unauthorized: {
		title: 'Access Denied',
		description: 'You don\'t have permission to access this resource. Please log in with the appropriate account.',
		icon: 'i-lucide:shield-x',
		category: 'authentication',
		actions: [
	...
	server_error: {
		title: 'Server Error',
		description: 'Our servers are experiencing issues. Please try again in a few minutes. If the problem persists, contact support.',
	...
	unknown: {
		title: 'Unknown Error',
		description: 'An unknown error occurred. Please try again or contact support if the problem persists.',
...
export function mapSupabaseError(error: any): string {
	...
	if (message.includes('invalid_grant') || message.includes('invalid token')) {
		...
		if (message.includes('verification') || message.includes('confirm')) return 'invalid_verification_link';
		return 'auth_error';
	}
	
	if (message.includes('expired') || code === 'token_expired') {
		if (message.includes('reset')) return 'expired_reset_link';
		if (message.includes('verification') || message.includes('confirm')) return 'expired_verification_link';
