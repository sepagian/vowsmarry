				...
				toastOptions.action = {
					...
					onClick: async (event: MouseEvent) => {
						...
						try {
							...
							this.submitError(retryError instanceof Error ? retryError.message : 'Retry failed', {
								...options,
			...
			toastOptions.action = {
				...
				onClick: async (event: MouseEvent) => {
					...
					try {
						...
						this.networkError(retryError instanceof Error ? retryError.message : 'Retry failed', {
							...options,
 ...
 */
export function showEmptyFormError(options?: {
	formName?: string;
	requiredFields?: string[];
	scrollToFirstField?: () => void;
}): void {
...
export function validateFormNotEmpty(
	...
	for (const field of requiredFields) {
		...
		if (
			...
			const displayName =
				options?.fieldDisplayNames?.[field] ||
				field
					.replace(/([A-Z])/g, ' $1')
					.replace(/_/g, ' ')
					.replace(/\b\w/g, (l) => l.toUpperCase())
