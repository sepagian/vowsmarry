...
class UnifiedToastService implements ToastService {
    ...
    public readonly form: FormToastMethods = {
        ...
        validationError: (errors: string[] | { field: string; message: string; displayName?: string }[]) => {
        ...
        emptyFormError: (options?: { formName?: string; requiredFields?: string[]; scrollToFirstField?: () => void }) => {
    ...
    private getCrudMessage(
        ...
        switch (operation) {
            case 'create':
                switch (type) {
                    case 'loading': return `Creating${entityName}...`;
                    case 'success': return `${entity ? entity.charAt(0).toUpperCase() + entity.slice(1) : 'Item'} created successfully`;
                    case 'error': return `Failed to create${entityName}`;
                }
            case 'update':
                switch (type) {
                    case 'loading': return `Updating${entityName}...`;
                    case 'success': return `${entity ? entity.charAt(0).toUpperCase() + entity.slice(1) : 'Item'} updated successfully`;
                    case 'error': return `Failed to update${entityName}`;
                }
            case 'delete':
                switch (type) {
                    case 'loading': return `Deleting${entityName}...`;
                    case 'success': return `${entity ? entity.charAt(0).toUpperCase() + entity.slice(1) : 'Item'} deleted successfully`;
                    case 'error': return `Failed to delete${entityName}`;
                }
            case 'fetch':
                switch (type) {
                    case 'loading': return `Loading${entityName}...`;
                    case 'success': return `${entity ? entity.charAt(0).toUpperCase() + entity.slice(1) : 'Data'} loaded successfully`;
                    case 'error': return `Failed to load${entityName}`;
                }
            default:
                switch (type) {
