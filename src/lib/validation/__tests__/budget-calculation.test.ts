...
describe('Budget Calculation Schema', () => {
    ...
    describe('Invalid Calculations', () => {
        it('should reject when remaining amount does not match calculation', () => {
            ...
            if (!result.success) {
                expect(result.error.issues[0].message).toContain('remaining amount does not match budget minus spent');
            }
        });
        ...
        it('should reject when category budget totals do not match overall budget', () => {
            ...
            if (!result.success) {
                expect(result.error.issues[0].message).toContain('Category totals do not match overall budget totals');
            }
        });
        ...
        it('should handle multiple categories with complex calculations', () => {
            const validData = {
                ...
                categories: [
                    ...
                    {
                        name: 'Catering',
                        budgetAmount: 35000.50,
                        actualAmount: 30000.25
                    },
                    {
                        name: 'Photography',
                        budgetAmount: 25000.75,
                        actualAmount: 15000.50
                    },
                    {
                        name: 'Decoration',
